...
SMTP credentials are read only from `CAREERFLOW_SMTP_*` environment variables at send time. Passwords are never written to the workspace. Delivery receipts contain the sender, recipient, subject, attachment hash and SMTP host, but never the credential.
