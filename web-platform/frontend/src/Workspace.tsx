...
type Artifact = { id: string; kind: string; size_bytes: number; active: boolean; version_id: string; created_at: string }
type DocumentChange = { region_id: string; old_text: string; new_text: string; evidence_ids: string[] }
type DeliveryReady = { requires_selection: false; recipient: string; subject: string; attachment_name: string; body: string; eml_artifact_id: string }
type DeliverySelection = { requires_selection: true; recipients: string[] }
type Delivery = DeliveryReady | DeliverySelection
...
export default function Workspace() {
  ...
  const canApprove = app.stage === 'drafted' && (!app.requires_document_plan || Boolean(app.document_plan))
  ...
  async function deleteApplication() {
    if (!window.confirm('确定永久删除这个岗位及其全部文件吗？')) return
    await action(() => api(`/api/v1/applications/${id}`, { method: 'DELETE' }), '岗位已删除。')
    navigate('/')
  }
  return <section className="workspace">
    ...
    {tab === 'draft' && <div className="panel">
      ...
      {app.document_plan && <details className="plan-review"><summary>审阅原格式修改计划（审批包的一部分）</summary><div className="change-list">{(app.document_plan.changes as DocumentChange[]).map(change => <article key={change.region_id}><header><b>{change.region_id}</b><span>{change.old_text.length} → {change.new_text.length} 字符</span></header><div className="change-columns"><div><small>原文</small><p>{change.old_text}</p></div><div><small>新文</small><p>{change.new_text}</p></div></div><p className="muted">证据：{change.evidence_ids.length ? change.evidence_ids.join('、') : '无（不可审批的重要表述）'}</p></article>)}</div></details>}
      <div className="actions">{draft && <Button variant="secondary" onClick={() => { void action(() => api(`/api/v1/applications/${id}/draft`, { method: 'PUT', body: JSON.stringify({ bundle: draft, document_plan: app.document_plan || null }) }), '修改已保存；旧审批已失效。') }}>保存修改</Button>}{draft && !app.document_plan && <Button variant="secondary" onClick={() => { void action(() => run('document-plan', { provider, session_id: sessionId, idempotency_key: idempotencyKey('document-plan'), template_id: 'default' }), '原格式修改计划已生成。') }}>生成原格式修改计划</Button>}{app.requires_document_plan && !app.document_plan && <Notice tone="error">上传过源简历的岗位必须先生成并审阅原格式修改计划，之后才能审批。</Notice>}<Button disabled={!canApprove || busy} onClick={() => { void action(() => run('approve'), '审批包已锁定。') }}>确认全部内容</Button><Button disabled={!canBuild || busy} onClick={() => { const endpoint = app.document_plan ? 'document-build' : 'build'; void action(() => run(endpoint), '构建完成。') }}>构建最终文件</Button></div>
    </div>}
    {tab === 'delivery' && <div className="panel"><h2>投递准备</h2><p>CareerFlow 只准备邮件，不连接或代发邮箱。</p><div className="artifact-list">{artifacts.map(item => <button key={item.id} className={item.active ? '' : 'inactive'} onClick={() => { void downloadArtifact(item.id) }}>{item.kind}<small>{item.active ? '当前版本' : '历史版本'} · {Math.ceil(item.size_bytes / 1024)} KB</small></button>)}</div><Button disabled={!canDeliver} onClick={() => { void action(() => api<Delivery>(`/api/v1/applications/${id}/delivery${recipient ? `?recipient=${encodeURIComponent(recipient)}` : ''}`), '邮件材料已生成。').then(value => { if (value) { setDelivery(value); if (!value.requires_selection) setRecipient(value.recipient) } }) }}>生成邮件预览</Button>{delivery?.requires_selection && <div className="stack"><Notice>JD中识别到多个邮箱，请明确选择收件人。</Notice><Field label="收件人"><select value={recipient} onChange={e => setRecipient(e.target.value)}><option value="">请选择</option>{delivery.recipients.map(item => <option key={item} value={item}>{item}</option>)}</select></Field></div>}{delivery && !delivery.requires_selection && <div className="email-preview"><b>收件人：</b>{delivery.recipient}<br/><b>主题：</b>{delivery.subject}<br/><b>附件：</b>{delivery.attachment_name}<pre>{delivery.body}</pre><button type="button" onClick={() => { void downloadArtifact(delivery.eml_artifact_id) }}>下载 .eml</button></div>}<Button disabled={app.stage !== 'built'} onClick={() => { void action(() => run('submitted'), '已记录为用户自行投递。') }}>我已经自行发送</Button></div>}
